    ...
    return (
      <main className="min-h-screen bg-[#0e0e10] p-10 text-white">
        <div className="mx-auto max-w-5xl">
          ...
          <Link href="/login" className="inline-block rounded-lg border border-gray-600 px-4 py-2 text-gray-200 hover:bg-gray-800">
            Go to Login
          </Link>
        </div>
      </main>
    ...
    return (
      <main className="min-h-screen bg-[#0e0e10] p-10 text-white">
        <div className="mx-auto max-w-5xl">
          ...
          <Link href="/projects" className="inline-block rounded-lg border border-gray-600 px-4 py-2 text-gray-200 hover:bg-gray-800">
            Back
          </Link>
        </div>
      </main>
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-8 flex items-start justify-between">
          ...
          <div className="flex gap-3">
            <Link href="/projects" className="rounded-lg border border-gray-600 px-4 py-2 text-gray-200 hover:bg-gray-800">
              ← Back
            </Link>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="rounded-lg border border-blue-500/40 bg-blue-500/10 px-4 py-2 text-blue-300 hover:bg-blue-500/20"
            >
              Print Material List
            </a>
          </div>
        ...
        <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3">
          <div className="rounded-xl border border-white/10 bg-white/5 p-5">
            ...
            <div className="mt-2 text-4xl font-semibold">{money(project.final_price)}</div>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/5 p-5">
            <div className="text-sm text-white/60">Expected Profit</div>
            <div className="mt-2 text-4xl font-semibold">{money(project.expected_profit)}</div>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/5 p-5">
            <div className="text-sm text-white/60">Margin</div>
            <div className="mt-2 text-4xl font-semibold">{pct(project.target_margin)}</div>
          </div>
        </div>
        ...
        <div className="mb-6 rounded-xl border border-white/10 bg-white/5 p-5">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
            ...
            <div><div className="text-white/50">Material Type</div><div className="font-medium">{project.material_type || "—"}</div></div>
            <div><div className="text-white/50">Railing Type</div><div className="font-medium">{project.railing_type || "—"}</div></div>
            <div><div className="text-white/50">Stair Count</div><div className="font-medium">{project.stair_count ?? "—"}</div></div>
            <div><div className="text-white/50">Job Type</div><div className="font-medium">{project.job_type || "new_build"}</div></div>
          </div>
        </div>
        ...
        <div className="mb-6 rounded-xl border border-white/10 bg-white/5 p-5">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-3">
            <div><div className="text-white/50">Material Cost</div><div className="font-medium">{money(project.material_cost)}</div></div>
            <div><div className="text-white/50">Labor Cost</div><div className="font-medium">{money(project.labor_cost)}</div></div>
            <div><div className="text-white/50">Permit Cost</div><div className="font-medium">{money(project.permit_cost)}</div></div>
            <div><div className="text-white/50">Equipment Cost</div><div className="font-medium">{money(project.equipment_cost)}</div></div>
            <div><div className="text-white/50">Overhead Cost</div><div className="font-medium">{money(project.overhead_cost)}</div></div>
            <div><div className="text-white/50">Total Job Cost</div><div className="font-medium">{money(project.total_job_cost)}</div></div>
          </div>
        </div>

        {/* ── Material Takeoff — client component ── */}
        <MaterialTakeoff
        ...
        <div className="mt-6 rounded-xl border border-white/10 bg-white/5 p-5">
          ...
          <div className="text-sm text-white/80">{project.notes?.trim() ? project.notes : "—"}</div>
        </div>
