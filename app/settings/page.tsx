...
type SettingsRow = {
  ...
  standard_assumptions: string; // UI: newline-delimited; stored as TEXT[]
  standard_exclusions: string;  // UI: newline-delimited; stored as TEXT[]
};
...
export default function SettingsPage() {
  ...
  const [form, setForm] = useState<SettingsRow>({
    ...
    crew_size: 2,
    crew_hourly_rate: 65,
    labor_method: "per_sqft",
    waste_factor: 1.10,
    auto_hardware: false,
    concrete_bag_price: 8.50,
    joist_hanger_price: 1.75,
    post_base_price: 12.00,
    hurricane_tie_price: 2.50,
    lag_bolt_price: 0.75,
    ...
    async function load() {
      ...
      if (data) {
        setForm({
          ...
          // Phase 4
          crew_size:            Number(data.crew_size            ?? 2),
          crew_hourly_rate:     Number(data.crew_hourly_rate     ?? 65),
          labor_method:         data.labor_method                ?? "per_sqft",
          waste_factor:         Number(data.waste_factor         ?? 1.10),
          auto_hardware:        Boolean(data.auto_hardware),
          concrete_bag_price:   Number(data.concrete_bag_price   ?? 8.50),
          joist_hanger_price:   Number(data.joist_hanger_price   ?? 1.75),
          post_base_price:      Number(data.post_base_price      ?? 12.00),
          hurricane_tie_price:  Number(data.hurricane_tie_price  ?? 2.50),
          lag_bolt_price:       Number(data.lag_bolt_price       ?? 0.75),
          proposal_expiry_days: Number(data.proposal_expiry_days ?? 30),
          standard_assumptions: Array.isArray(data.standard_assumptions)
            ? (data.standard_assumptions as string[]).join("\n")
            : (data.standard_assumptions ?? ""),
          standard_exclusions: Array.isArray(data.standard_exclusions)
            ? (data.standard_exclusions as string[]).join("\n")
            : (data.standard_exclusions ?? ""),
        });
  ...
  function setBool<K extends keyof SettingsRow>(key: K, value: boolean) {
    setForm((prev) => ({ ...prev, [key]: value }));
  }
  ...
  async function save() {
    ...
    // Parse textarea fields back to arrays, filtering blank lines
    ...
    const payload = {
      ...
      crew_hourly_rate:     form.crew_hourly_rate,
      labor_method:         form.labor_method,
      waste_factor:         form.waste_factor,
      auto_hardware:        form.auto_hardware,
      concrete_bag_price:   form.concrete_bag_price,
      joist_hanger_price:   form.joist_hanger_price,
      post_base_price:      form.post_base_price,
      hurricane_tie_price:  form.hurricane_tie_price,
      lag_bolt_price:       form.lag_bolt_price,
      proposal_expiry_days: form.proposal_expiry_days,
      standard_assumptions: toArray(form.standard_assumptions),
      standard_exclusions:  toArray(form.standard_exclusions),
