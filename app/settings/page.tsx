...
export default function SettingsPage() {
  ...
  async function handleLogoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!allowed.includes(file.type)) {
      setLogoErr("Only PNG, JPG, and SVG files are allowed.");
      return;
    }

    // Validate size (5MB)
    if (file.size > 5 * 1024 * 1024) {
      setLogoErr("File must be under 5MB.");
      return;
    }
  ...
  async function handleLogoRemove() {
    ...
    // Try removing both png and jpg variants
  ...
  return (
    <main className="min-h-screen bg-[#0e0e10] p-10 text-white">
      <div className="mx-auto max-w-4xl">
        ...
        {msg ? (
          <div className="mb-4 rounded-lg border border-emerald-500/40 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-200">{msg}</div>
        ) : null}
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
          ...
          <p className="mb-5 text-xs text-white/40">
            Your logo and contact info appear on every PDF proposal sent to clients.
          </p>
          ...
          <div className="mb-6 rounded-xl border border-white/10 bg-[#111827] p-5">
            ...
            {logoUrl ? (
              <div className="flex items-start gap-5">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <button
                    ...
                    className="rounded-lg border border-white/20 px-3 py-1.5 text-xs text-white hover:bg-white/10 disabled:opacity-50"
                  >
              ...
              <div
                onClick={() => fileInputRef.current?.click()}
                className="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-white/15 bg-white/5 px-6 py-8 transition hover:border-white/25 hover:bg-white/8"
              >
            ...
            {logoErr ? (
              <p className="mt-2 text-xs text-red-400">{logoErr}</p>
            ) : null}
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => updateTextField("company_website", e.target.value)}
                className="w-full rounded-lg border border-white/15 bg-[#111827] px-3 py-2"
                placeholder="www.smithdeckbuilders.com"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => updateTextField("company_address", e.target.value)}
                className="w-full rounded-lg border border-white/15 bg-[#111827] px-3 py-2"
            ...
            <div className="md:col-span-2 rounded-xl border border-blue-500/20 bg-blue-500/5 p-4">
              ...
              <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
                <div>
                  ...
                  <div className="text-sm font-medium text-white">${(form.pt_material_rate * selectedRegion.materialMult).toFixed(2)}/sqft</div>
                </div>
                <div>
                  <div className="text-xs text-white/50">Trex</div>
                  <div className="text-sm font-medium text-white">${(form.trex_material_rate * selectedRegion.materialMult).toFixed(2)}/sqft</div>
                </div>
                <div>
                  <div className="text-xs text-white/50">TimberTech</div>
                  <div className="text-sm font-medium text-white">${(form.timbertech_material_rate * selectedRegion.materialMult).toFixed(2)}/sqft</div>
                </div>
                <div>
                  <div className="text-xs text-white/50">Labor</div>
                  <div className="text-sm font-medium text-white">${(form.labor_rate_per_sqft * selectedRegion.laborMult).toFixed(2)}/sqft</div>
                </div>
              </div>
            </div>
          </div>
